...
def create_rocket_stl(filename="rocket_model.stl", length=100, radius=10):
    ...
    for fin_num in range(num_fins):
        ...
        add_quad(
            inner_top,
            inner_bottom,
            bottom_circle[fin_num * num_segments // num_fins],
        ...
        add_triangle(
            ...
            [
                inner_bottom[0]
                + math.cos(fin_angle + math.pi / 2) * fin_thickness * 0.5,
                inner_bottom[1]
                + math.sin(fin_angle + math.pi / 2) * fin_thickness * 0.5,
                inner_bottom[2],
            ],
        )
    ...
    for i in range(num_segments):
        next_i = (i + 1) % num_segments
        add_triangle(
            center_bottom, engine_bottom_circle[i], engine_bottom_circle[next_i]
        )
